...
Each YAML uses the corresponding `session_runner` variant (e.g., `github/parallelworks/interactive_session@main` resolves to the appropriate deployment). The differences are typically in scheduler directives, partition names, and cluster-specific environment setup in the `inputs.sh` generation.
