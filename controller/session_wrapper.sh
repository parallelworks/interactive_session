...
if [[ "$USERMODE" == "k8s" || "$NEW_USERCONTAINER" == "0" ]];then
    ...
    TUNNELCMD="ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${USER_CONTAINER_HOST} \"ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -N -L 0.0.0.0:$openPort:localhost:\$servicePort ${controller}\""
else
