...
if [ -z "$servicePort" ]; then
    SERVER_TUNNEL_CMD="ssh ${ssh_usercontainer_options} -fN -R 0.0.0.0:$openPort:0.0.0.0:\$servicePort ${USER_CONTAINER_HOST}"
else
    SERVER_TUNNEL_CMD="ssh ${ssh_usercontainer_options} -fN -R 0.0.0.0:$openPort:0.0.0.0:$servicePort ${USER_CONTAINER_HOST}"
fi
...
sshusercontainer="ssh ${ssh_usercontainer_options} -f ${USER_CONTAINER_HOST}"
