...
if [ -z ${service_vnc_exec} ] || ! [ -f "${service_vnc_exec}" ]; then
    ...
    service_vnc_exec="singularity exec --writable-tmpfs --bind /tmp/.X11-unix:/tmp/.X11-unix --bind ${HOME}:${HOME} --bind /etc/machine-id:/etc/machine-id --bind /var/lib/dbus/machine-id:/var/lib/dbus/machine-id ${service_vncserver_sif} vncserver -xstartup ~/.vnc/xstartup.turbovnc"
