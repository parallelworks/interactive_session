...
if [ -z ${service_vnc_exec} ] || ! [ -f "${service_vnc_exec}" ]; then
    ...
    singularity_binds="--bind /etc/passwd --bind /etc/group --bind /etc/profile.d --bind /etc/ld.so.conf.d --bind /etc/bash.bashrc.local --bind /etc/cray-pe.d --bind /etc/machine-id --bind /etc/profile --bind /p --bind /app --bind /opt/cray --bind /opt/pbs --bind /var --bind /usr/local/lib --bind /usr/local/lib64 --bind /usr/share/lmod --bind /opt/cray/pe/modules --bind /etc/krb5.conf --bind /usr/local/bin --bind /etc/pbs.conf --bind /pbs --bind /run --bind /etc/zprofile --bind /etc/zshrc --bind /etc/zshenv --bind /usr/share/zsh"
